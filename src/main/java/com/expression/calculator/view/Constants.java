package com.expression.calculator.view;

public class Constants {
    public static final String MAIN_MENU_MESSAGE = "Для начала работы калькулятора введите выржание." + "\n"
            + "Для завершения работы программы введите quit" + "\n"
            + "Введите значение выражения: ";

    public static final String EXIT_MESSAGE = "Работа программы завершена!";
    public static final String OPERATION_EXCEPTION_MESSAGE = "Введен не корректный символ! Допустим только +, -, *, /, (, )";
    public static final String USER_INPUT_EXCEPTION_MESSAGE = "Ошибка чтения с консоли";
    public static final String QUIT_COMMAND = "quit";
    public static final String RESULT_MESSAGE = "\nResult: ";
}
